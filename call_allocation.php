                    ...
                    data: {
                        ...
                        data: [
                            ...
                            if (isset($_SESSION['type_center']) && $_SESSION['type_center']) {
                                ...
                                $stmt = $obj->con1->prepare("SELECT tbl.*,t1.name AS technician_name FROM (SELECT c1.*,s1.name AS service_center_name,p1.name AS product_category_name, c2.barcode, c2.warranty AS warranty_status, CONCAT(c2.fname,' ',c2.lname) AS customer_name,c2.contact AS customer_contact, CONCAT(c1.allocation_date, ' ', c1.allocation_time) AS allocation_datetime FROM call_allocation c1,customer_reg c2,service_center s1,product_category p1 WHERE c1.complaint_no=c2.complaint_no AND c2.warranty!=2 AND c2.service_type!=16 AND c1.service_center_id=s1.id AND c2.product_category=p1.id AND s1.id=?) AS tbl LEFT JOIN technician t1 on tbl.technician=t1.id order by tbl.id DESC");
                                ...
                                $stmt = $obj->con1->prepare("SELECT tbl.*,t1.name AS technician_name FROM (SELECT c1.*,s1.name AS service_center_name,p1.name AS product_category_name, c2.barcode, c2.warranty AS warranty_status, CONCAT(c2.fname,' ',c2.lname) AS customer_name, c2.contact AS customer_contact, CONCAT(c1.allocation_date, ' ', c1.allocation_time) AS allocation_datetime FROM call_allocation c1, customer_reg c2, service_center s1, product_category p1 WHERE c1.complaint_no=c2.complaint_no AND c2.warranty!=2 AND c2.service_type!=16 AND c1.service_center_id=s1.id AND c2.product_category=p1.id) AS tbl LEFT JOIN technician t1 ON tbl.technician=t1.id ORDER BY tbl.id DESC");
                                $stmt->execute();
                                $Resp = $stmt->get_result();
                                $stmt->close();
                                $i = 1;
