...
// call list
    public function call_list($date,$type,$technician_id)
    {
        if($type=="pending")
        {
            $stmt = $this->con->prepare("SELECT c1.*,c2.*,s1.name as service_center_name,pc1.name as product_category_name,s1.address as service_center_address,s1.contact as service_contact,a1.name as service_area_name FROM customer_reg c1,call_allocation c2,service_center s1,product_category pc1,service_area a1 WHERE c2.complaint_no=c1.complaint_no and c2.service_center_id=s1.id and c1.product_category=pc1.id and s1.area=a1.id and c2.status='pending'  and c2.technician=?");
            $stmt->bind_param("i",$technician_id);
        }
