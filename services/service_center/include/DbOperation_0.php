...
//service center login
  ...
  public function insert_service_center_device($vid,$token,$type)
    ...
    // home page
    public function homepage($date,$service_center_id)
    {
    ...
    public function call_list($date,$type,$service_center)
    {
        if($type=="pending")
        {
            $stmt = $this->con->prepare("SELECT * FROM customer_reg c1,call_allocation c2 WHERE c2.complaint_no=c1.complaint_no and c2.status=? and  c2.service_center_id=?");
            $stmt->bind_param("si", $type,$service_center);
        }
        else if($type=="allocated")
        {
            $stmt = $this->con->prepare("SELECT * FROM customer_reg c1, call_allocation c2 WHERE c2.complaint_no=c1.complaint_no and c2.status=? and  c2.service_center_id=? and c2.complaint_no not in (select complaint_no from call_history )");
            $stmt->bind_param("si", $type,$service_center);
        }
        else if($type=="closed")
        {
            $stmt = $this->con->prepare("SELECT * FROM customer_reg c1, call_allocation c2 WHERE c2.complaint_no=c1.complaint_no and ( c2.status=? or c2.status='cancelled') and c1.date >= '".date('Y-m-01')."' and c1.date<='".date('Y-m-t')."' and c2.service_center_id=?");
            $stmt->bind_param("si", $type,$service_center);
        }
        else
        {
            
            $stmt = $this->con->prepare("SELECT * FROM customer_reg c1, call_allocation c2 WHERE c2.complaint_no=c1.complaint_no and c2.status=? and c2.service_center_id=? and c2.status='new'");
            $stmt->bind_param("si", $type,$service_center);
